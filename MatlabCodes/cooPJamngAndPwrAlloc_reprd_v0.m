...
R_sec_vec = zeros(size(eavesdynPos));%preallocation (2020/02/10)
...
%     angles_SJ = atan(y_jam_dist./x_jam_dist);
    ...
    R_sec_summer = 0;%secrecy capacity agreggator (2020/02/10)
...
%         gamma_RD = P_R*abs(h_RD).^2./sigmaSqrd;% When gammas not fixed    
        ...
        R_sec_term1 = 0.5*log(1+(1-alpha_)*gamma_SE);
        R_sec_term2 = 0.5*log((1+beta_ * gamma_RD)*(1+(1-beta_)*gamma_RE));
        R_sec_term3 = 0.5*log((1+gamma_SE)*(1 + gamma_RE));
        R_sec = R_sec_term1 + R_sec_term2 - R_sec_term3;
        ...
        R_sec_summer = R_sec_summer + R_sec
    end
    R_sec_vec(eavesd_dist_iter) = R_sec_summer/iter;%average of the secrecy capacity after the iterations
end
...
%semilogy(eavesdynPos,R_sec,'-o');%plot of SNR gap in dB
